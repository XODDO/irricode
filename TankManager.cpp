...
unsigned long duration, depth_in_cm = 0;
   ...
   digitalWrite(trigger_and_echo_pins[0], HIGH);     delayMicroseconds(10);   digitalWrite(trigger_and_echo_pins[0], LOW);
...
//.. the deeper the worse, the shallower, the better .. //
  ...
  if(depth_in_cm-int(top_dead_center+0.50) >= 0)   accurate_depth = 100.00 - (float(depth_in_cm)-top_dead_center)*100/float(max_depth);
