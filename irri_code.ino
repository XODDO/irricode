...
String Kit_ID = "MakRIF II IrriKit 001"; // --- "MakRIF II IrriKit 002", "MakRIF II IrriKit 001";
...
DHT sensor(DHTPIN_, DHTTYPE); const uint8_t temp_signal = 29;
...
void Setting() {
  ...
  LCD.setFont(u8g_font_helvR08);LCD.setPrintPos(3, 37);   LCD.print("2. Add SMS Receiver"); 
...
// --- the MAIN SECTION
   ...
   if(soil_Tank_field[1]==0) {tanka = "Not in!";} //:String(soil_Tank_field[1]+"%");
   if(soil_Tank_field[0]==0) {soyilo = "Not in!"; }//:String(soil_Tank_field[0]+"V");
    ...
    LCD.setFont(u8g_font_helvR08); 
    LCD.setPrintPos(15, 46);   LCD.print("Tank Level: "); LCD.setFont(u8g_font_timR08);LCD.print(tanka); 
    LCD.drawTriangle(60,58, 70,58, 64,63); // --- scroll down
       ...
       LCD.setPrintPos(14, 34);   LCD.print("Pump: ");  LCD.setFont(u8g_font_timR08);  LCD.print(PumpPrompt); 
       LCD.setFont(u8g_font_helvR08); 
       LCD.setPrintPos(14, 46);   LCD.print("Valve: ");  LCD.setFont(u8g_font_timR08); LCD.print(IrrigationPrompt); if(*count_down>0){LCD.print(*count_down);}
       LCD.drawTriangle(60,58, 70,58, 64,63); // --- scroll down
   }
...
//LCD.setFont(u8g_font_helvR08);
  ...
  LCD.setPrintPos(53, 49); LCD.print("Percent: "); LCD.print(*current_amount); LCD.print("%");
...
// ===  save to local storage
  ...
  unsigned int dataPoints = 0;
            dataPoints = sizeof(settings)/sizeof(settings[0]);
...
//    Serial.print("Writing to "); Serial.println(settings_file); 
        for(int i=0; i<dataPoints; i++){ if(i==0 || i==(dataPoints-1)){local_storage.println();} local_storage.println();
         for(int j=0; j<2; j++){
...
String binded_for_datalog[19][2] = {
                                    {"Kit Identity: ", Kit_ID},
...
unsigned int dataPoints = 0;
             dataPoints = sizeof(binded_for_datalog)/sizeof(binded_for_datalog[0]);
...
String Temp = irri_Parameter[0];
String Humi = irri_Parameter[1] + "%";
String Duration = irri_Parameter[2] + "hrs";
String IrrigationClock =  irri_Parameter[3];    
String Volume = irri_Parameter[4] + "liters";
String SoilMoisture1 = irri_Parameter[5] + "%";
String SoilMoisture2 = irri_Parameter[6] + "%";
    
   String binded_4_transmission[9][2] = {{"Data Round: ", String(message_counter)},
                                         {"Field Temp: ", Temp},
                                         {"Field Humidity: ", Humi},
                                                                             
                                         {"Irrigated at: ", IrrigationClock},
                                         {"Lasting for: ", Duration},
                                         {"Irrigated with: ", Volume},
                                         {"Soil Moisture b4 irrigation: ", SoilMoisture1},
                                         {"Soil Moisture at FC: ", SoilMoisture2}};

    unsigned int dataPoints = 0;
                 dataPoints = sizeof(binded_4_transmission)/sizeof(binded_4_transmission[0]);
    ...
    //if(already_sent == false){
      Serial.println("Sending via GSM Network...");
                 ...
                 for(int i=0; i<dataPoints; i++){ if(i==0 || i==(dataPoints-1)) SIM.println();
                     for(int j=0; j<2; j++){
...
exec.check_tank(tank_level_sensorPin);
...
pinMode(13, OUTPUT);
sensor.begin(); pinMode(temp_signal, INPUT);
...
for(int i=0; i<4; i++){ pinMode(soil_moisture_signal[i], OUTPUT); digitalWrite(soil_moisture_signal[i], HIGH);}

pinMode(humi_temp_signal, OUTPUT);   digitalWrite(humi_temp_signal, HIGH);
...
while(loader<100){ loader++; pos++; flip = !flip; //................................
 ...
 //initialize all pinModes 1 by one

       if(loader == 5){pinMode(power_indica, OUTPUT); pinMode(danger, OUTPUT); pinMode(full_beam, OUTPUT);  pinMode(dim, OUTPUT);    digitalWrite(full_beam, 1);}
  else if(loader == 6){pinMode(right, INPUT);  }
  else if(loader == 7){pinMode(left, INPUT); pinMode(back, INPUT); pinMode(up, INPUT);}     
  else if(loader == 8){pinMode(set, INPUT);}
  else if(loader == 9){pinMode(back, INPUT); pinMode(up, INPUT); }
  else if(loader == 10){pinMode(sound_fx, OUTPUT); pinMode(Okay, INPUT);   pinMode(down, INPUT);}
  //pinMode fans and sensing nodes
  
  else if(loader > 20 && loader <= 21){digitalWrite(danger, 1);  /*tone(sound_fx,988,300);*/}
  else if(loader > 21 && loader <= 22){     }
  else if(loader > 22 && loader <= 23){digitalWrite(danger, 0);  }

  else if(loader > 24 && loader <= 25){digitalWrite(power_indica, 1);  digitalWrite(soil_moisture_signal[1], LOW);/*tone(sound_fx,523,150);    */}
  else if(loader > 25 && loader <= 26){                               /*tone(sound_fx,831,500);/* tone(sound_fx,1000,200);  */}
  else if(loader > 26 && loader <= 28){digitalWrite(power_indica, 0);/*  tone(sound_fx,2100);     */}
  else if(loader == 29){noTone(sound_fx); }
  else if(loader >= 30 && loader < 40){digitalWrite(danger, flip);  /*tone(sound_fx,622,200);*/}
  else if(loader == 40){digitalWrite(danger, OFF); }
...
else if(loader>40 && loader<=68){ digitalWrite(power_indica, flip); digitalWrite(soil_moisture_signal[0], LOW);
...
LCD.firstPage();  
  ...
  //  LCD.drawBox(15, 55, pos, 7); LCD.drawFrame(15,55,100,7);
        ...
        if(loader == 85){digitalWrite(power_indica, OFF); digitalWrite(soil_moisture_signal[2], LOW);}
...
//bit one; 
bool valve_is_ON = false; bool was_irrigating = false;  //bool done = false;
bool pump_is_ON = false;

uint8_t water_percent = 0;

String irrigationPrompt = "Valve Not Opened";
String Pumping;
...
//---heavily event-driven program
  ...
  while(!set_up){  system_setup();  }
    ...
    if(hour == 18) save_unlocked = true;
    else {save_unlocked = false; data.already_saved = false;}

    if(hour == 18) send_unlocked = true;  // hour%2==0; send every two hours 
    else { send_unlocked = false; data.already_sent = false; }

 //if(current_Screen != 8){ //if not in power saving mode
...
// the time constraint thing --- how do we know it would be irrigating when time is not right???
         ...
         if(moisture_now > 21.50) {/* Do Nothing, if ON, power OFF */ digitalWrite(valvePin, HIGH); irrigationPrompt = "Soil Moisture OK";}
   ...
   if(count_down_in_secs == current_millis) {count_down_in_secs = 0; digitalWrite(valvePin, HIGH); valve_is_ON = false; }
        ...
        uint8_t hh_mm_ss[3] = {hour, minute, second};
         ...
         // --- get crop, pump, soil details from SD card --- //
          String crop_details[5] = {"Beans", "Flowering", String(temp), String(humi), "1/4/21"};
           temp = humi_temp(temp_signal);  humi = h; 
      ...
      if(send_unlocked == true){
         if(data.already_sent==false) {
              ...
              temp = humi_temp(temp_signal);  humi = h; // Sensor readings up to 2 seconds 'old' (its a very slow sensor)
...
//if power_status == HIGH ... 
         if(dim_screen >= 900 && dim_screen < 1000) digitalWrite(full_beam, LOW); //dim=true
    else if(dim_screen >= 1000) { digitalWrite(dim, LOW);
    ...
    else if(led_blink >= 2 && led_blink < 15) digitalWrite(power_indica, LOW);
    else {led_blink=0;}
   ...
   updateScreen(&current_Screen, &second);
   dim_screen++; return_home++; //sleepScreen+;
   
sub_routine++;
} // --- loop()


bool pump_ON = false; String Pump_Prompt = "Not Connected!";

String run_refill_pump(const uint8_t * pin, bool power_is_Available, bool tank_is_beyond_quarter, unsigned long levo_when_pumping){
   ...
   if(power_is_Available == false){Pump_Prompt = "Power Low";}//charge until 12.6V //blink in red and put dangreous icon --- njagala okupimpinga naye power
        ...
        if(water_percent >= 30){Pump_Prompt = "Tank not empty"; //already some water
            if(pump_ON)if(levo_when_pumping >= 100) {digitalWrite(*pin, HIGH); pump_ON = false; Pump_Prompt = "Now Full"; /*data.Save()*/}  //now full, record current volume
          }
        else{ //if(!pump_ON)Pump_Prompt = "Tank Empty";   //if quarter or below

            if(pump_ON == false) {digitalWrite(*pin, LOW); pump_ON = true; Pump_Prompt = "Pumping.. ";} // blink in green
            
        }
  }
     //Pump_Prompt = (pump_ON)?"Pumping":"Not pumping";
     //else {digitalWrite(pump, HIGH); pump_ON = false;}
    //return true;
    return Pump_Prompt;
}
...
String farmerName = "Patrick";
 String farm_size = "10ha";
 String farmLocation = "Rushozi";
...
if(isOn == false){
    ...
    if(isnan(h)) { h=0.00; t=0.00; ScreenMessage[0] = ScreenMessage[1] =  "Out!";      //
     ...
     if(in && !state){digitalWrite(indicator, 1); tone(sound_fx, 1500,50);   state = true; digitalWrite(indicator, 0);}
...
byte hour_min_sec[3] = {hour, minute, *sec};
 ...
 // --- normal mode --- //   
   ...
   /* 3. */       temp = humi_temp(temp_signal);  humi = h; 
...
//
 ...
 //  -------------- data.save(); //if time is due for data-logging
  // -------------- data.transmit(); // if the time due for data transmission
 ...
 */
  ...
  int menu_select(int scroller){
         if(scroller == 1) scroller = 11;
    else if(scroller == 2) scroller = 15;
    else if(scroller == 3) scroller = 16;
    else if(scroller == 4) scroller = 10;
    else if(scroller == 5) scroller = 29;
    else if(scroller == 6) scroller = 19;
...
if(current_Screen == 0){  // --- settings screen
  ...
  save = true;
        ...
        switch(_btn){
            ...
            case back: started = false; tone(sound_fx, 123, 30); current_Screen = SleepNow;  break;
            ...
            case Okay: started = true; break; // temporarily start the burner fan speed regulation
        ...
        switch(_btn){
            case set:  case Okay: 
                current_Screen = menu_select(Screen.dj_selekta); tone(sound_fx, 880, 30);   break;
            case back:  current_Screen = 9;  tone(sound_fx, 123, 30); break;
        ...
        switch(_btn){
            case set:  case Okay: current_Screen = (Screen.set_option==1?25:21); if(Screen.set_option==1){Screen.temp_hr = hour;  Screen.temp_min = minute;} break; // if backlit_state == false{digitalWrite(back_light, HIGH);}
            case back:  current_Screen = 10;  tone(sound_fx, 123, 30); break;
